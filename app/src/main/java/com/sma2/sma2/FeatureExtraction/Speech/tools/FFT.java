...
public class FFT extends FourierTransform
{
   ...
   */
    ...
    /*
    This  function was modified to compute the ACF. THe real part is redefined and
    the imaginary part is zero
     */
    public void inverseacf(float[] buffer)
    {

        real = new float[real.length];
        System.arraycopy(buffer,0,real,0,buffer.length-1);
        bitReverseComplex();
        fft();
        //float inv[] = new float[real.length];
        // copy the result in real into buffer, scaling as we do
        //buffer = new float[real.length];
        for (int i = 0; i < buffer.length/2; i++)//we only use half of the autocorrelation
        {
            buffer[i] = real[i] / real.length;
        }
    }
    ...
    /*** Compute ACF using Fast Fourier Transform.
     @param x is the data.
     @return ac is the result
     ***/
    public float[] acf(float [] x)
    {
        float[] temp = new float[timeSize];
        System.arraycopy(x,0,temp,0,x.length);
        forward(temp);// X = FFT(x)
        float[] ac = new float[specSize()];
        for (int i =0;i<specSize();i++) {
            ac[i] = getBand(i)*getBand(i);//Power density |X|^2
        }
        inverseacf(ac);//real(IFFT(|X|^2))
        return ac;
    }
    ...
    private void buildReverseTable(int N)
    {
        //int N = timeSize;
        reverse = new int[N];
    ...
    private void buildTrigTables(int N)
    {
        sinlookup = new float[N];
        coslookup = new float[N];
        for (int i = 0; i < N; i++)
